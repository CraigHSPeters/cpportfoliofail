...
function zerogravity_scripts_styles() {
	...
	// Toggle search
	wp_enqueue_script('zerogravity-scripts-functions', get_template_directory_uri() .'/js/zerogravity-scripts-functions.js', array('jquery'), ZEROGRAVITY_VERSION, true);
}

/**
 * Add preconnect for Google Fonts.
 *
 * @since ZeroGravity 2.0.0
 *
...
function zerogravity_resource_hints( $urls, $relation_type ) {
	if ( wp_style_is( 'zerogravity-fonts', 'queue' ) && 'preconnect' === $relation_type ) {
...
add_filter( 'wp_resource_hints', 'zerogravity_resource_hints', 10, 2 );
...
if ( !function_exists('zerogravity_the_posts_pagination') ){
	function zerogravity_the_posts_pagination(){

		echo '<div class="posts-pagination-wrapper">';
		the_posts_pagination(array(
				'mid_size' => 2,
				'prev_text' => __('&laquo; Previous', 'zerogravity'),
				'next_text' => __('Next &raquo;', 'zerogravity'),
			));
		echo '</div>';
